...
int set_iomap(void) {
  ...
  // mmap registers (FC):
  mm =
      mmap(0, FC_REG_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, fd, FC_REG_BASE);
...
bool load_program() {
  ...
  if (!dfs.is_open()) {
    ...
    *(volatile unsigned int *)(iomap_cnv + 0x0080) =
        0;  // Program starts at address 0
    do {
      word = read32_le(dfs);
      *(volatile unsigned int *)(iomap_cnv + 0x0084) =
          word;  // Write word and increment address
      // cout << hex << word << "\t";
      // if (n%32 == 31)
      // cout << "\n";
      n++;
    ...
    // cout << "End address = " << hex << (*(volatile unsigned int*)(iomap_cnv +
    // 0x0080))*4 << "\n";
    dfs.close();
    return true;
  }
...
void get_hw_info() {
  ...
  unsigned int fi;
      ...
      volatile unsigned long *)(iomap_cnv +
                                ((0x109) << 2));  // read ffrequency information
...
void get_info(unsigned int v, unsigned int *ret) {
  unsigned int fi;
      ...
      volatile unsigned long *)(iomap_cnv +
                                ((0x109) << 2));  // read frequency information
  switch (v) {
    case FREQ_CONV:
      *ret = fi & 0xFF;
      break;
...
enum { sanityRW_addr = 0x106 * 4, buttonChk_addr = 0x107 * 4 };

void reset_button_state() {
  *(volatile unsigned int *)(iomap_cnv + buttonChk_addr) =
      0;  // clear button sticky bits
}

unsigned int get_button_state() {
  unsigned int button = *(
      volatile unsigned int
          *)(iomap_cnv + buttonChk_addr);  // read and clear button sticky bits
