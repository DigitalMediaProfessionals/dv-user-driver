...
void CDMP_Network::verbose(bool en)
{
	_verbose = en;
}
...
{
	//Load IQ table
	unsigned short *addr = reinterpret_cast<unsigned short *>(fc_layer.sw.weight_addr);

	for (int i = 0; i < 256; i++) {
		*(volatile unsigned int*)(dmp::modules::get_iomap_fc () + 0x004C) = (2 << 28) | (1 << 24) | (i << 16) | addr[i];       // matrix vertical
	}
}
...
bool CDMP_Network::load_weights(const std::string& filename)
{
	...
	int bytes_read = 0;
	if(_verbose)
		std::cout << "Loading CONV/IP  Weights." << std::endl;
	...
	if(_verbose)
	{
		std::cout << bytes_read << " bytes read." << " [DONE]" << std::endl;
	}

	dfs.close();

	if (fc_layers.size())
		load_IQ_table(fc_layers[0]);
...
static int memory_alloc(int fd, bool _verbose,
				        ...
				        std::vector<unsigned long> &reserved_memory_addresses_cpu)
{
	...
	for (auto i = mem_req_vec.begin(); i != mem_req_vec.end(); i++)
	{
		...
		//int mem_size_pages = (mem_size / page_size) + ((mem_size % page_size) == 0 ? 0 : 1);
	...
	for (int i = 0; i < N; i++)
	{
		reserved_memory_addresses_cpu[i] = reserved_memory_addresses_fpga[i] - SYS_DDR_BASE_PA + iomap_ddr;
		if(_verbose)
			std::cout << std::hex << i << "\t" <<  reserved_memory_addresses_fpga[i] << "\t-> "  << reserved_memory_addresses_cpu[i] << "\n";
	}
...
bool CDMP_Network::reserve_memory(bool set_addr) {
  ...
  int error = memory_alloc(fdC, _verbose, memory_size_request, reserved_memory_addresses_fpga, reserved_memory_addresses_cpu);
...
static void remap(unsigned short *src, unsigned short *dst, int x_size, int y_size, int channel_size)
{
...
static void remap_hw(unsigned short *src, unsigned short *dst, int x_size, int y_size, int channel_size)
{
	for (int y = 0; y < y_size; y++)
	{
		for (int x = 0; x < x_size; x++)
...
static void run_conv(top_conv_conf *conf, int t_sleep)
{
	...
	*(volatile unsigned int*)(iomap_cnv + 0x0080) = 0x2000; // Conf. struct starts at 0x2000
	//int conf_size = sizeof(configuration)/4 + ((sizeof(configuration) & 0x3) == 0 ? 0 : 1);
	int conf_size_bytes = hw_conf_size(conf);
	int conf_size = conf_size_bytes / 4 + ((conf_size_bytes & 0x3) != 0); // words
...
static void run_ip(top_fc_conf *fc)
{
	...
	*(volatile unsigned int*)(iomap_fc + 0x0044) = 0x00000223;               // Raw vector, quant weights, raw bias as separate input
	*(volatile unsigned int*)(iomap_fc + 0x0048) = fc->hw.actfunc;           // w*v, tanh activation function
	*(volatile unsigned int*)(iomap_fc + 0x0050) = fc->hw.input_size;        // matrix horizontal size
	*(volatile unsigned int*)(iomap_fc + 0x0054) = fc->hw.output_size;       // matrix vertical
	*(volatile unsigned int*)(iomap_fc + 0x0058) = fc->hw.output_base_addr;  // output write address
	*(volatile unsigned int*)(iomap_fc + 0x0074) = fc->hw.input_base_addr;   // input read address
	*(volatile unsigned int*)(iomap_fc + 0x007C) = fc->hw.stride;            // stride
	*(volatile unsigned int*)(iomap_fc + 0x0080) = fc->hw.weight_addr;       // weight address
	*(volatile unsigned int*)(iomap_fc + 0x0060) = fc->hw.bias_addr;         // bias address
	*(volatile unsigned int*)(iomap_fc + 0x0064) = fc->hw.bias_size;         // bias size (in bytes)
...
static void run_flatten(fpga_layer &layer)
{
	if (!layer.is_input_hw_layout)
		return;
	unsigned short *src = reinterpret_cast<unsigned short *>(layer.addr_cpu_input);
	unsigned short *dst = reinterpret_cast<unsigned short *>(layer.addr_cpu_output);
	int x_size = layer.input_dim[0];
	int y_size = layer.input_dim[1];
	int channel_size = layer.input_dim[2];
	remap(src, dst, x_size, y_size, channel_size);
...
static void run_copy_concat(fpga_layer &layer, int input_layer_num, fpga_layer **input_layers)
{
	...
	{
		unsigned short *src = reinterpret_cast<unsigned short *>(input_layers[i]->addr_cpu_output);
		...
		while (src_copied_size < src_channel_size)
		{
			...
			unsigned short *dst_copy = dst + (dst_copied_size / chunk_size) * chunk_stride + (dst_copied_size % chunk_size);
			unsigned short *src_copy = src + (src_copied_size / chunk_size) * chunk_stride + (src_copied_size % chunk_size);
...
void CDMP_Network::run_network(int t_sleep)
{
	...
	for(unsigned int i = 0; i < num_layers; i++)
	{
		...
		case LT_FLATTEN:
			...
			break;
		case LT_COPY_CONCAT:
			run_copy_concat(layers[i], layers[i].input_layer_num, layers[i].input_layers);
			break;
		case LT_CUSTOM:
		...
		if (dump_output)
		{
			...
			if (i == 0)
			{
				...
				fwrite(layers[i].addr_cpu_input, 1, layers[i].input_dim[0] * layers[i].input_dim[1] * 6, fout);
				fclose(fout);
			}
		}
#endif
...
void CDMP_Network::get_final_output(std::vector<float>& out, unsigned int i)
{
	...
	if (output_layers[i]->is_f32_output)
		memcpy((void*)(&out.front()), output_layers[i]->addr_cpu_output, output_size * 4);
	else
		halfp2singles((void*)(&out.front()), output_layers[i]->addr_cpu_output, output_size);
}
...
int CDMP_Network::get_convolution_performance(int layerID)
{
	...
	if(conv_layers.size())
		return get_conv_layer(layerID).sw.output.performance;
...
int CDMP_Network::get_innerproduct_performance(int layerID)
{
	...
	if(fc_layers.size())
		return get_ip_layer(layerID).sw.performance;
...
void get_layer_input(fpga_layer &layer, std::vector<float> &layer_input)
{
	...
	{
		unsigned short *src = reinterpret_cast<unsigned short *>(layer.addr_cpu_input);
		unsigned short *dst = reinterpret_cast<unsigned short *>(&layer_input.front());
		int x_size = layer.input_dim[0];
		int y_size = layer.input_dim[1];
		int channel_size = layer.input_dim[2];
		dst += input_size;
		...
		halfp2singles((void*)(&layer_input.front()), layer.addr_cpu_input, input_size);
}
...
void put_layer_output(fpga_layer &layer, std::vector<float> &layer_output, bool is_output_hw_layout)
{
	...
	{
		unsigned short *src = reinterpret_cast<unsigned short *>(&layer_output.front());
		unsigned short *dst = reinterpret_cast<unsigned short *>(layer.addr_cpu_output);
...
void CDMP_Network::SetLayerAddresses() {
  for (unsigned int i = 0; i < num_conv_layers; i++) {
    conv_layers[i].hw.input.input_base_addr += reserved_memory_addresses_fpga[1];
    conv_layers[i].hw.output.output_base_addr += reserved_memory_addresses_fpga[1];
    if (conv_layers[i].hw.output.eltwise_base_addr != 0xDEADBEEF) {
      conv_layers[i].hw.output.eltwise_base_addr += reserved_memory_addresses_fpga[1];
    }
    int run_num = conv_conf_num_runs(&conv_layers[i]);
    for (int j = 0; j < run_num; j++) {
      conv_layers[i].hw.run[j].weight_base_addr += reserved_memory_addresses_fpga[0];
    }
  }
  for (unsigned int i = 0; i < num_fc_layers; i++) {
    fc_layers[i].sw.weight_addr = reinterpret_cast<void *>(fc_layers[i].hw.param_base_addr + reserved_memory_addresses_cpu[0]);
  ...
  for (unsigned int i = 0; i < num_layers; i++) {
    layers[i].addr_cpu_input = reinterpret_cast<void*>(reserved_memory_addresses_cpu[1] + layers[i].addr_offset_input);
    layers[i].addr_cpu_output = reinterpret_cast<void*>(reserved_memory_addresses_cpu[1] + layers[i].addr_offset_output);
  }
