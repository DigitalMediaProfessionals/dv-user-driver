...
class CDMP_Network {
 private:
  /*!
          ...
          @details If enabled, various information, such as memory size request,
     allocation etc. are displayed.
          */
  bool _verbose;
 ...
 protected:
  ...
  /*!
          @brief Vector containing list of memory size requested by the network
     (i.e. to store the weights, the temporary  buffers during inference etc.)
          */
 ...
 public:
  ...
  int get_convolution_performance(
      int layerID =
          -1);  //-1 means sum of all layers, other specific convolution id
...
void put_layer_output(fpga_layer &layer, std::vector<float> &layer_output,
                      bool is_output_hw_layout = false);
