...
typedef struct conv_config_impl {
  ...
  bool operator <(const struct conv_config_impl& pt) const {
    return std::make_tuple(width, height, n_channels) <
        std::make_tuple(pt.width, pt.height, pt.n_channels);
  }
} conv_config;
...
int test_cmdlists(const std::vector<conv_config*>& confs) {
  ...
  for (auto it = confs.begin(); it != confs.end(); ++it) {
    conv_config *conf = *it;
    snprintf(prefix, sizeof(prefix), "data/%dx%d/%d",
             conf->width, conf->height, conf->n_channels);
    LOG(" %s", prefix);
  }
  LOG("\n");
  ...
  for (auto it = confs.begin(); it != confs.end(); ++it) {
    ...
    if (!conf->hash_set) {
      ...
      conf->caffe_output = (__fp16*)malloc(conf->n_channels * conf->height * conf->width * sizeof(__fp16));
      n = fread(conf->caffe_output, sizeof(__fp16), conf->n_channels * conf->height * conf->width, fin);
      fend = feof(fin) || ((fread(&c, 1, 1, fin) == 0) && (feof(fin)));
      fclose(fin);
      if (n != conf->n_channels * conf->height * conf->width) {
        ERR("fread() returned %d while expecting %d for %s\n", n, conf->n_channels * conf->height * conf->width, fnme);
  ...
  for (auto it = confs.begin(); it != confs.end(); ++it) {
    ...
    if (conf->hash_set) {  // check hash
      ...
      SHA256_CTX sha256;
      SHA256_Init(&sha256);
      SHA256_Update(&sha256, conf->io_ptr + o_offs, conf->width * conf->height * conf->n_channels * sizeof(__fp16));
      ...
      outp.resize(conf->width * conf->height * conf->n_channels);
      std::set<int> offsets;
      ...
      if (!conf->failed) {  // compute hash
        const int output_size = conf->height * conf->width * conf->n_channels * sizeof(__fp16);
...
int main(int argc, char **argv) {
  ...
  while ((dir = readdir(d))) {
    ...
    if (sscanf(fnme, "%d%d", &config.width, &config.height) != 2) {
      continue;
    }
    ...
    while ((subdir = readdir(subd))) {
      ...
      if (sscanf(fnme, "%d", &config.n_channels) != 1) {
        continue;
      }

      char prefix[256];
      snprintf(prefix, sizeof(prefix), "data/%dx%d/%d",
               config.width, config.height, config.n_channels);
